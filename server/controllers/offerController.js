...
export async function getFullOffer(req, res, next) {
   try {
      ...
      const full = adaptFullOfferToClient(offer, offer.author);
      return res.status(200).json(full);
   } catch (error) {
      return next(ApiError.internal('Ошибка при получении полного оффера'));
   }
}
...
export async function createOffer(req, res, next) {
   try {
      const {
         ...
         commentsCount, latitude, longitude, userId
      } = req.body;
